...
const fields = function (args, ct) { return args.length === ct ? args[ct - 1].join(', ') : '*' }
...
const mysqlStore = function (sql) {
  const st = {
    ...
    insert: curry(function (type, record) {
      return sql.query('INSERT INTO ?? (??) VALUES (?)', [ table(type), keys(record), values(record) ])
        .then(function (x) {
          return st.find(type, x.insertId)
        })
    }),
    insertMany: curry(function (type, records) {
      let recKeys = keys(records[0])
